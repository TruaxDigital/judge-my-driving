...
const PLANS = [
  ...
  },
  {
    ...
    addonPrice: 89,
    icon: Truck,
    description: 'For small fleets and businesses',
  ...
  },
  {
    ...
    addonPrice: 79,
    icon: Truck,
    description: 'For growing fleets that need insights',
    features: ['25 stickers included', 'Everything in Starter Fleet', 'Monthly PDF reports', 'Custom sticker branding', 'Priority email support', '3 admin seats'],
    popular: true,
    fleet: true,
    color: 'border-primary',
  },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {PLANS.filter(p => !p.fleet).map(plan => (
            <PlanCard
              ...
              current={user?.plan_tier === plan.id}
...
function PlanCard({ plan, loading, current, onSubscribe }) {
  ...
  return (
    <div className={cn(
      ...
      <p className="text-xs text-muted-foreground">Additional stickers: ${plan.addonPrice}/yr each</p>
      ...
      >
        {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : current ? 'Current Plan' : 'Subscribe'}
      </Button>
    </div>
  );
}