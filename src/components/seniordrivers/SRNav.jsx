    ...
    <>
      <nav style={{
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 32 }} className="sr-desktop-nav">
            {links.map(l => (
              <a
                key={l.label}
                href={l.href}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {links.map(l => (
              <a
                key={l.label}
                href={l.href}
                onClick={e => { e.preventDefault(); smoothScroll(l.href); setMenuOpen(false); }}
      ...
      <style>{`
        @media (max-width: 768px) { .sr-desktop-nav { display: none !important; } .sr-mobile-nav { display: block !important; } }
        @media (min-width: 769px) { .sr-desktop-nav { display: flex !important; } .sr-mobile-nav { display: none !important; } }
