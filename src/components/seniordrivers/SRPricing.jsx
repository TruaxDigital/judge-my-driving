...
const FAMILY_FEATURES = [
  ...
  'Shared dashboard so siblings can see the same data',
...
export default function SRPricing() {
  ...
  const handleCheckout = async (planId) => {
    ...
    if (!isAuthed) {
      base44.auth.redirectToLogin(`/senior-drivers?plan=${planId}`);
      return;
    }
    setLoading(planId);
  ...
  return (
    <section id="pricing" style={{ backgroundColor: '#1A1A1A', padding: '96px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div className="sr-pricing-grid">
          ...
          <div style={{
            ...
            >
              {loading === 'individual'
                ? <Loader2 size={18} style={{ animation: 'sr-spin 1s linear infinite' }} />
        ...
        <p style={{ textAlign: 'center', color: '#B8B8B8', fontSize: 15, marginTop: 32 }}>
          Coordinating with caregivers? You can invite a sibling to the dashboard from your account settings.
        </p>

        <div className="sr-trust-chips">
      ...
      <style>{`
        ...
        @media (max-width: 768px) { .sr-pricing-grid { grid-template-columns: 1fr; } }
      `}</style>
    </section>
  );
}