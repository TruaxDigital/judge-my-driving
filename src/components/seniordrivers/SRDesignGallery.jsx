...
const SENIOR_DESIGNS = [
  ...
  { id: 'tell_my_kids', label: "How's My Driving? Tell My Kids" },
  { id: 'tell_my_mom', label: "How's My Driving? Tell My Mom" },
  { id: 'tell_my_dad', label: "How's My Driving? Tell My Dad" },
  ...
  { id: 'new_driver', label: 'Got Feedback?' },
  ...
  return (
    ...
    >
      <div style={{ aspectRatio: '3/2', overflow: 'hidden' }}>
        <img src={DESIGN_URLS[id]} alt={label} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
...
export default function SRDesignGallery() {
  const [lightbox, setLightbox] = useState(null);

  return (
    <section id="designs" style={{ backgroundColor: '#0F0F0F', padding: '96px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 700, lineHeight: 1.15, marginBottom: 16 }}>
            Designs that respect your parent
          </h2>
          <p style={{ color: '#B8B8B8', fontSize: 16, lineHeight: 1.6, maxWidth: 480, margin: '0 auto' }}>
            No "elderly driver" cliches. Pick from 15+ designs that read confident, not condescending. Switch designs anytime from your dashboard for $19.
          </p>
        </div>

        <div className="sr-gallery-container">
          {SENIOR_DESIGNS.map(({ id, label }) => (
        ...
        {lightbox && (
          ...
          >
            ...
            >
              <img src={DESIGN_URLS[lightbox.id]} alt={lightbox.label} style={{ width: '100%', display: 'block' }} />
              <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <p style={{ fontSize: 16, fontWeight: 600, color: '#FFFFFF', margin: 0 }}>{lightbox.label}</p>
                <button
                  onClick={() => setLightbox(null)}
                  style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: 8, color: '#B8B8B8', fontSize: 13, fontWeight: 600, padding: '6px 14px', cursor: 'pointer' }}
                >
                  Close
                </button>
              </div>
      ...
      <style>{`
        ...
        @media (max-width: 1024px) { .sr-gallery-container { grid-template-columns: repeat(4, 1fr); } }
        @media (max-width: 768px) {
          ...
          .sr-gallery-container::-webkit-scrollbar { height: 4px; }
          .sr-gallery-container::-webkit-scrollbar-thumb { background: rgba(212,160,23,0.4); border-radius: 2px; }
          .sr-design-tile { min-width: 160px !important; scroll-snap-align: start; }
        }
      `}</style>
    </section>
  );
