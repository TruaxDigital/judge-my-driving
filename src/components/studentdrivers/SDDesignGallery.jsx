...
const TEEN_DESIGNS = [
  ...
  { id: 'student_driver', label: 'Student Driver. Score My Skills.' },
...
export default function SDDesignGallery() {
  ...
  return (
    <section id="designs" style={{ backgroundColor: '#0F0F0F', padding: '96px 24px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: 700, lineHeight: 1.15, marginBottom: 16 }}>
            Pick the sticker that fits your teen
          </h2>
          <p style={{ color: '#B8B8B8', fontSize: 16, lineHeight: 1.6, maxWidth: 480, margin: '0 auto' }}>
            15+ designs to choose from. Subtle, funny, or direct. Switch designs anytime from your dashboard for $19.
          </p>
        </div>

        <div className="sd-gallery-container">
          {TEEN_DESIGNS.map(({ id, label }) => (
      ...
      <style>{`
        ...
        @media (max-width: 1024px) { .sd-gallery-container { grid-template-columns: repeat(4, 1fr); } }
        @media (max-width: 768px) {
          ...
          .sd-gallery-container::-webkit-scrollbar { height: 4px; }
          .sd-gallery-container::-webkit-scrollbar-thumb { background: rgba(212,160,23,0.4); border-radius: 2px; }
          .sd-design-tile { min-width: 160px !important; scroll-snap-align: start; }
        }
      `}</style>
    </section>
  );
